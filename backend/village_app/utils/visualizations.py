...
def generate_3d_model_image(village_id, output_path):
    try:
    ...
    base_dir = "C:/Users/prash/Desktop/rural dev cmd/backend/outputs/models"
    os.makedirs(base_dir, exist_ok=True)
    output_path = os.path.join(base_dir, f"{village_id}_3d_model.png").replace("\\", "/")
...
num_trees = max(min({num_trees}, 40), 20)
...
"""
    ...
    try:
        area_scale = village.village_area ** 0.5 * 5
        num_houses = int(village.current_census_population / 5)
        green_cover_area = (village.green_cover / 100) * village.village_area
        num_trees = int(green_cover_area * 20)
        num_schools = village.number_of_schools
        num_parks = village.parks
        num_playgrounds = village.playgrounds
        num_lakes = village.infrastructure.get('lakes', 0)
        street_lighting = village.street_lighting

        print(f"area_scale: {area_scale}")
        print(f"num_houses: {num_houses}")
        print(f"num_trees: {num_trees}")
        print(f"num_schools: {num_schools}")
        print(f"num_parks: {num_parks}")
        print(f"num_playgrounds: {num_playgrounds}")
        print(f"num_lakes: {num_lakes}")
        print(f"street_lighting: {street_lighting}")
        ...
        blender_script = blender_script.format(
            ...
            num_parks=num_parks,
            num_playgrounds=num_playgrounds,
            num_lakes=num_lakes,
            street_lighting=street_lighting,
            output_path=output_path
        )

        blender_script_path = os.path.join(base_dir, "generate_village_model.blend.py").replace("\\", "/")
        with open(blender_script_path, 'w') as f:
            f.write(blender_script)

        print(f"Blender script generated at: {blender_script_path}")
        print("To render the 3D model, run the following command in Blender:")
        print(f"blender -b -P {blender_script_path}")
