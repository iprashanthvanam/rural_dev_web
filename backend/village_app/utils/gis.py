...
#         gdf = gpd.read_file("data/telangana.geojson")
...
#         minx, miny = village.longitude - 0.03, village.latitude - 0.03
#         maxx, maxy = village.longitude + 0.03, village.latitude + 0.03
...
#         village_point = Point(village.longitude, village.latitude)
...
def generate_map_image(village, output_path):
    ...
    try:
        ...
        # Filter for roads, buildings, parks, water bodies, and landmarks (SAFE VERSION)
        gdf_roads = gdf[gdf['highway'].notna()] if 'highway' in gdf.columns else gpd.GeoDataFrame()
        gdf_buildings = gdf[gdf['building'].notna()] if 'building' in gdf.columns else gpd.GeoDataFrame()
        gdf_parks = gdf[gdf['landuse'] == 'park'] if 'landuse' in gdf.columns else gpd.GeoDataFrame()
        gdf_gardens = gdf[gdf['amenity'] == 'garden'] if 'amenity' in gdf.columns else gpd.GeoDataFrame()
        gdf_water = gdf[gdf['natural'] == 'water'] if 'natural' in gdf.columns else gpd.GeoDataFrame()
        ...
        landmarks = []
        if village.infrastructure['lakes'] > 0:
            landmarks.append(('Lake', '🏞️', Point(village.longitude - 0.01, village.latitude + 0.01)))
        if village.infrastructure['temples'] > 0:
            landmarks.append(('Temple', '🛕', Point(village.longitude, village.latitude)))
        if village.number_of_schools > 0:
            landmarks.append(('School', '🏫', Point(village.longitude + 0.01, village.latitude - 0.01)))
        landmarks_gdf = gpd.GeoDataFrame(landmarks, columns=['Feature', 'Symbol', 'geometry'], geometry='geometry', crs="EPSG:4326")

        # Recommended features to plot (only if they don't exist)
        recommended_features = []
        if village.number_of_hospitals == 0:
            recommended_features.append(('Hospital', '🏥', Point(village.longitude - 0.015, village.latitude - 0.015)))
        if not village.post_office_availability:
            recommended_features.append(('Post Office', '📮', Point(village.longitude + 0.015, village.latitude + 0.015)))
        if village.petrol_bunks == 0:
            recommended_features.append(('Petrol Bunk', '⛽', Point(village.longitude - 0.01, village.latitude)))
        if village.parks == 0:
            recommended_features.append(('Park', '🌳', Point(village.longitude + 0.02, village.latitude + 0.02)))
        if village.playgrounds == 0:
            recommended_features.append(('Playground', '🏞️', Point(village.longitude + 0.02, village.latitude - 0.02)))
        if not village.street_lighting:
            recommended_features.append(('Street Lighting', '💡', Point(village.longitude, village.latitude + 0.01)))
        if not village.renewable_energy_source:
            recommended_features.append(('Solar Panels', '☀️', Point(village.longitude - 0.02, village.latitude)))
        if not village.waste_management_everyday:
            recommended_features.append(('Dumping Yard', '🗑️', Point(village.longitude + 0.01, village.latitude)))
        if not village.network_connectivity:
            recommended_features.append(('Signal Tower', '📡', Point(village.longitude - 0.015, village.latitude + 0.015)))
        if not village.market_availability:
            recommended_features.append(('Market Yard', '🏪', Point(village.longitude + 0.015, village.latitude - 0.015)))
        if not village.banks_atm_facility:
            recommended_features.append(('Bank/ATM', '🏧', Point(village.longitude - 0.02, village.latitude - 0.02)))
        if not village.public_transport:
            recommended_features.append(('Bus Bay', '🚌', Point(village.longitude + 0.01, village.latitude + 0.02)))
        ...
        # Plot village location with emoji
        village_gdf.plot(ax=ax, color='red', marker='o', markersize=120)
        ax.text(village.longitude, village.latitude + 0.003, '🏘️', fontsize=20, ha='center', va='center')
        ax.text(village.longitude, village.latitude - 0.003, village.name, fontsize=8, ha='center', va='top')
